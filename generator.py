...
def generate_adafruit_font_file(symbols, font_name="MyFont", pt_size=24):
    ...
    for code in range(first_ascii, last_ascii + 1):
        ...
        if char == " ":
            ...
            glyphs.append(f"    {{ {current_offset}, 0, 0, {space_width}, 0, 0 }}, // ASCII {code:3} (0x{code:02X}) ' '")
            continue

        # 2. PROCESS ACTUAL SYMBOLS
...
# Execution
user_input = input("Enter the characters you want (or type 'All' for 32-255):\n")

if user_input.strip().lower() == "all":
    symbols = "".join(chr(i) for i in range(32, 256))
else:
    symbols = user_input
...
print(f"Success! '{output_file}' generated with {len(symbols)} characters.")
import subprocess
import re
